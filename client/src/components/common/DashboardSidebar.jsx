...
const DashboardSidebar = () => {
    ...
    const planName = user?.subscriptionPlan === "family" ? "Family" : user?.subscriptionPlan === "plus" ? "Plus" : "Free";
    ...
    return (
        <aside className={`${isCollapsed ? 'w-20' : 'w-64'} transition-all duration-300 flex-shrink-0 border-e border-kashf-border bg-kashf-bg flex flex-col h-full overflow-y-auto scrollbar-tight`}>
            ...
            <div className={`p-4 border-t border-kashf-border space-y-4 shrink-0 flex flex-col ${isCollapsed ? 'items-center' : ''}`}>
                {/* Plan Indicator */}
                {!isCollapsed && (
                    <div className="p-3 bg-neutral-900 rounded-lg border border-neutral-800 transition-all">
                        ...
                        <div className="space-y-1">
                            <div className="flex justify-between text-xs">
                                <span className="text-neutral-400">Coins Usage</span>
                                <span className="text-neutral-200">{usedCoins} / {totalCoins}</span>
                            </div>
