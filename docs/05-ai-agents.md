...
Simulation state snapshot
  ...
  └─ user consumptionGoals (monthlyKwhLimit, targetBillEgp)
         │
         ▼
  Groq API prompt (engineered template)
         │
         ▼
...
| :--- | :--- | :--- | :--- | :--- |
| **P0** | AI Consumption Advisor | Medium | High | Groq API key, prompt template |
| **P1** | Tier Prediction Agent | Low | High | Needs only math — no AI call |
| **P1** | Smart Recommendations | Medium | High | Activity log analysis + Groq |
| **P2** | What-If Agent | Medium | Medium | Clone runtime + fast-forward tick |
| **P2** | Natural Language Agent | High | High | Groq intent classification |
